...
class StandardUser(UserProfile):
    ...
    def create_post(self, content):
        ...
        if len(self.post) >= self.post_limit:
            raise AttributeError(f"You've reached your limit. your limit is {self.post_limit}")
        return


class PermiumUser(UserProfile):
    def __init__(self, username, email, sign_up_date, post_limit):
        super().__init__(username, email, sign_up_date)

        self.post_limit = post_limit
        self.post = []

    def create_post_permium(self, content):
...
pru = PermiumUser("ali", "ali@email.com", "july 2024", "No Limit")





pru.create_post_permium("hello wordl")
